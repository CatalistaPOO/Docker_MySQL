...
select cl.nombre_cliente,concat(emp.apellido1,' ', emp.apellido2,', ', emp.nombre) as empleado_a_cargo, ofi.ciudad as ciudad_oficina
from clientes as cl 
left join pagos pg on pg.fk_cliente = cl.id_cliente
join empleados as emp on cl.fk_empleado_rep_ventas = emp.id_empleado
join oficinas as ofi on emp.fk_oficina = ofi.id_oficina
...
from clientes as cl 
left join pagos pg on pg.fk_cliente = cl.id_cliente
left join pedidos as pd on cl.id_cliente = pd.fk_cliente
where pg.id_transaccion is null and pd.id_pedido is null;
...
from clientes as cl
join pagos as pg on cl.id_cliente = pg.fk_cliente;
...
select prod.nombre, prod.precio_venta
from productos as prod
where prod.precio_venta = (select max(precio_venta) from productos);
...
oficinas y el número de empleados que tiene.
...
select pd.id_pedido, cl.id_cliente, pd.fecha_esperada, pd.fecha_entrega
from pedidos as pd
join clientes as cl on pd.fk_cliente = cl.id_cliente
...
select prod.nombre, dp.fk_pedido
from productos as prod
left join detalles_pedido as dp on prod.id_producto = dp.fk_producto
where dp.fk_pedido is null;
...
from clientes as cl
where cl.fk_empleado_rep_ventas is null;
...
select concat(emp.nombre,' ',emp.apellido1,', ',emp.apellido2) as n_empleado, puesto
from empleados as emp
left join clientes as cl on emp.id_empleado = cl.fk_empleado_rep_ventas
where fk_empleado_rep_ventas is null;
...
select id_cliente,ciudad,fk_empleado_rep_ventas
from clientes
where ciudad='Madrid' and (fk_empleado_rep_ventas = 11 or fk_empleado_rep_ventas = 30);
...
select puesto, concat(nombre,' ', apellido1,' ', apellido2) as n_jefe
from empleados
...
select id_cliente
from clientes 
where id_cliente not in (select fk_cliente from pagos) and id_cliente in (select fk_cliente from pedidos);
...
select concat(emp.nombre,' ',emp.apellido1,' ', emp.apellido2) as n_empleado, emp.puesto, ofi.telefono, emp.extension-- falta telefono
from empleados as emp
join oficinas as ofi on fk_oficina = ofi.id_oficina
...
select cl.nombre_cliente, emp.nombre as nombre_representante, pg.id_transaccion
from clientes as cl
join empleados as emp on cl.fk_empleado_rep_ventas = emp.id_empleado
join pagos as pg on cl.id_cliente = pg.fk_cliente
where pg.fk_cliente is not null;
...
-- MAL, faltaria ver la suma
select prod.id_producto, prod.nombre, dp.cantidad
from productos as prod
join detalles_pedido as dp on prod.id_producto = dp.fk_producto
where cantidad >=all (select cantidad from detalles_pedido);
-- BIEN
...
select *
from empleados as emp
where emp.fk_oficina is null and id_empleado not in (select fk_empleado_rep_ventas from clientes);
...
select id_oficina, codigo_postal,ciudad
from oficinas;
...
select cl.nombre_cliente as n_cliente, emp.nombre as n_empleado
from clientes as cl
join empleados as emp on cl.fk_empleado_rep_ventas = emp.id_empleado
left join pagos as pg on cl.id_cliente = pg.fk_cliente
where pg.fk_cliente is null;
...
from detalles_pedido;
-- IVA
select  sum(cantidad * precio_unidad) as base_imponible, round(sum(cantidad * precio_unidad )* 0.21,2) as IVA
from detalles_pedido;
...
select nombre, apellido1, apellido2, email, fk_jefe
from empleados
where fk_jefe = 7 ;
...
select  ofi.linea_direccion1, ofi.linea_direccion2, cl.region
from oficinas as ofi
join empleados as emp on fk_oficina = ofi.id_oficina
join clientes as cl on fk_empleado_rep_ventas = emp.id_empleado
where cl.region = 'Fuenlabrada';
...
select sum(dp.cantidad) * count(pd.id_pedido)
from detalles_pedido as dp
join pedidos as pd on dp.fk_pedido = pd.id_pedido;
...
select emp.nombre, emp.apellido1, emp.apellido2, emp.email, jefe.nombre, jefe.apellido1
from empleados as emp
join empleados as jefe on emp.fk_jefe = jefe.id_empleado
...
select * 
from productos as prod
where id_producto in (select fk_producto from detalles_pedido);
...
select prod.id_producto,prod.nombre, sum(dp.cantidad)as cantidades
from productos as prod
join detalles_pedido as dp on prod.id_producto = dp.fk_producto
group by id_producto
order by cantidades desc limit 20;
...
select cl.nombre_cliente, emp.nombre,emp.apellido1, ofi.ciudad
from clientes as cl
join empleados as emp on cl.fk_empleado_rep_ventas = emp.id_empleado
join oficinas as ofi on emp.fk_oficina = ofi.id_oficina;
...
select id_cliente
from clientes
where id_cliente in (select fk_cliente from pagos where year(fecha_pago) = 2008);
...
from clientes as c
join pagos as p on c.id_cliente = p.fk_cliente
group by c.id_cliente, c.nombre_cliente, c.nombre_contacto, c.apellido_contacto
order by c.nombre_cliente;
...
from clientes as cl
where cl.limite_credito > (select sum(total) as total_gasto
							from pagos
                            where fk_cliente = cl.id_cliente);

-- 52 Devuelve un listado con el nombre de los empleados junto con el nombre de sus jefes.
select e.nombre, e.fk_jefe, j.nombre
from empleados as e
join empleados as j on e.fk_jefe = j.id_empleado;

-- 53 ¿Cuántos pedidos hay en cada estado? Ordena el resultado de forma descendente por el número de pedidos.
select pd.estado, count(pd.id_pedido)as n_pedidos
from pedidos as pd
group by pd.estado
order by n_pedidos desc;

-- 54 Devuelve las oficinas donde no trabajan ninguno de los empleados que hayan sido
-- los representantes de ventas de algún cliente 
...
-- 56 Devuelve un listado que muestre solamente los empleados que
-- no tienen un cliente asociado junto con los datos de la oficina donde trabajan.
select emp.id_empleado, cl.id_cliente, ofi.*
from empleados as emp
left join clientes as cl on emp.id_empleado = cl.fk_empleado_rep_ventas
join oficinas as ofi on emp.fk_oficina = ofi.id_oficina
where cl.fk_empleado_rep_ventas is null;
...
select prod.nombre, prod.cantidad_en_stock
from productos as prod
where prod.cantidad_en_stock = (select min(cantidad_en_stock) from productos);

-- 58 Calcula cuántos clientes tiene cada una de las ciudades que empiezan por M
select distinct(count(cl.id_cliente)), cl.ciudad
from clientes as cl
...
select avg(pg.total) as pago_medio_2009
from pagos as pg
where year(pg.fecha_pago) = 2009;

-- 61 Devuelve el nombre del cliente con mayor límite de crédito.
select cl.nombre_cliente, cl.limite_credito
from clientes as cl
where cl.limite_credito >= (select max(limite_credito) from clientes);
...
select cl.nombre_cliente, count(pd.id_pedido)
from clientes as cl
join pedidos as pd on cl.id_cliente = pd.fk_cliente
group by id_cliente;
...
select e.id_empleado, e.nombre as nombre_empleado,j.id_empleado,  j.nombre as nombre_jefe,jj.id_empleado, jj.nombre as jefe_del_jefe
from empleados as e
left join empleados as j on e.fk_jefe = j.id_empleado
left join empleados as jj on j.fk_jefe = jj.id_empleado
order by jefe_del_jefe;

-- 66 Muestre la suma total de todos los pagos que se realizaron para cada uno de los años que aparecen en la tabla pagos.
select sum(total), year(fecha_pago)
from pagos 
...
-- 69 Devuelve un listado con el código de pedido, código de cliente, fecha esperada y fecha de entrega de los pedidos
...
from pedidos as pd
join clientes as cl on pd.fk_cliente = cl.id_cliente
where adddate(pd.fecha_entrega,2) <= pd.fecha_esperada;

select pd.id_pedido, cl.id_cliente, pd.fecha_esperada, pd.fecha_entrega
from pedidos as pd
join clientes as cl on pd.fk_cliente = cl.id_cliente
where datediff(pd.fecha_esperada,pd.fecha_entrega) >= 2;

-- Aunque devuelve lo mismo, no es correcto restar fechas de la misma manera que lo es sumarlas, etc
-- datediff es la manera correcta de hacer la resta de dos fechas.
select pd.id_pedido, cl.id_cliente, pd.fecha_esperada, pd.fecha_entrega,fecha_esperada + fecha_entrega as incorrecto
from pedidos as pd
join clientes as cl on pd.fk_cliente = cl.id_cliente
...
select cl.id_cliente, cl.nombre_cliente, coalesce(sum(pg.total), 0) as total
from clientes as cl
left join pagos as pg on cl.id_cliente = pg.fk_cliente
group by id_cliente
order by total;
...
-- no se puede hacer, enunciado mal
...
select dp.fk_pedido, prod.nombre,dp.cantidad, sum(dp.cantidad) cant, sum(dp.cantidad * dp.precio_unidad) as total
from detalles_pedido dp
join productos as prod on prod.id_producto = dp.fk_producto
group by id_producto
having total > 3000;

-- 76 Calcula el número de productos diferentes que hay en cada uno de los pedidos.
select count(fk_producto) as cant,fk_pedido
from detalles_pedido
group by fk_pedido
order by cant desc;
...
select cl.nombre_cliente, pd.fecha_esperada,pd.fecha_entrega
from clientes as cl
join pedidos as pd on cl.id_cliente = pd.fk_cliente
where datediff(pd.fecha_entrega, pd.fecha_esperada) >= 1; 
...
select id_cliente,concat(c.nombre_contacto, c.apellido_contacto)as n_cliente,telefono, c.ciudad,c.pais, pg.fecha_pago, pg.total, pg.id_transaccion
from clientes as c
...
--  La vista deáber tener las siguientes columnas: nombre y apellidos del cliente concatendados, teléfono, ciudad, pais, código del pedido, fecha del pedido, fecha esperada, fecha de entrega y 
-- la cantidad total del pedido, 
-- que será la suma del producto de todas las cantidades por el precio de cada unidad, que aparecen en cada línea de pedido.
create or replace view listado_pedidos_clientes as 
select id_cliente, concat(c.nombre_contacto, c.apellido_contacto)as n_cliente,telefono, c.ciudad,c.pais,pd.id_pedido,
		pd.fecha_pedido,pd.fecha_esperada, pd.fecha_entrega,sum(dp.cantidad * dp.precio_unidad)as total,pd.estado
from clientes as c
join pedidos as pd on c.id_cliente = pd.fk_cliente
join detalles_pedido as dp on pd.id_pedido = dp.fk_pedido
group by pd.id_pedido;

-- 82 Utilice las vistas que ha creado en los pasos anteriores para devolver
-- un listado de los clientes de la ciudad de Madrid que han realizado pagos.
select id_cliente, n_cliente, ciudad, id_transaccion
from listado_pagos_clientes 
where ciudad  = 'Madrid';

-- 83 Utilice las vistas que ha creado en los pasos anteriores para devolver
-- un listado de los clientes que todavía no han recibido su pedido.
select id_cliente, n_cliente,estado
from listado_pedidos_clientes
where estado = 'Entregado';

-- 84 Utilice las vistas que ha creado en los pasos anteriores para calcular
--  el número de pedidos que se ha realizado cada uno de los clientes.
select id_cliente, count(*) as cant_pedidos
from listado_pedidos_clientes
group by id_cliente;
...
select id_cliente, max(total) as mayor, min(total)as menor
from listado_pedidos_clientes
group by id_cliente;
